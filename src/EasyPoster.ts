...
function loadSchemaImages(
  ...
  function reParserImage(s: PosterStageSchema | PosterGroupSchema | PosterImageSchema | PosterTextSchema): void {
    if (s.type === 'image') {
...
function createPoster(schema: PosterStageSchema, progress?: (v: number) => void): Promise<PosterStage> {
  return new Promise((resolve) => {
